    ...
    public static class WebHookReceiver
    {
            ...
            ILogger logger)
        {
            ...
            if (headers.TryGetValue(WebHookSignatureHeaderName, out var actualSignature))
            {
                ...
                var secretBytes = Encoding.UTF8.GetBytes(batchmain.WebHookSecret);
            ...
            var subscriptionKey = Environment.GetEnvironmentVariable("SpeechtoTextKey", EnvironmentVariableTarget.Process);
            var region = Environment.GetEnvironmentVariable("SpeechtoTextRegion", EnvironmentVariableTarget.Process);
            var endPoint = Environment.GetEnvironmentVariable("SpeechtoTextEndPoint", EnvironmentVariableTarget.Process);
